...
使用者管理服務上的檢視者角色，對於也被指派角色來檢視或管理支援案例的使用者而言，是一個常見指派的角色。這是因為如果帳戶擁有者在 IAM 設定中限制使用者清單的可見性，可能會導致使用者無法看到由帳戶中其他使用者開啟的支援案例。不過，如果他們獲指派使用者管理服務的檢視者角色，則該使用者清單可見性設定不會影響他們能否在帳戶中檢視案例。
{: tip}
...
您可以提供使用者存取權，以更新帳戶設定、檢視訂閱、檢視行銷方案、套用特性碼、更新消費限制，以及使用計費服務來追蹤用量。
