...
移至**管理** > **存取 (IAM)**，然後選取**使用者**。然後，從清單選取您的名稱或另一個使用者的名稱。您可以在「使用者」詳細資料頁面上尋找該使用者的 IAM ID 以及其電子郵件位址。
...
標準基礎架構的存取權是從使用者開始。如需相關資訊，請參閱[管理標準基礎架構存取](/docs/iam?topic=iam-mngclassicinfra)。

如果您需要指派對啟用 IAM 功能之基礎架構服務（例如 {{site.data.keyword.vpc_full}}）的存取權，請從**存取原則**標籤指派存取權給使用者或存取群組。
