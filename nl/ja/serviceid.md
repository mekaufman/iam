...
サービス ID の削除が原因でサービスのユーザーに中断や停止が発生する状態を避けるために、UI または CLI を使用してサービス ID をロックするオプションがあります。 サービス ID をロックすることにより、ポリシーの変更、削除、割り当てを防止することもできます。 サービス ID をロックできることに加え、アカウント内に作成する各サービス ID に関連付けられている[個々の API キーをロック](/docs/iam?topic=iam-lockkey#lockkey)できます。
